...
for tests that define an an accuracy metric
(i.e., those that use the \texttt{analytics} fixture).
In this study, we focus on four such tests,
which we claim are all necessary components
of large-scale cognitive brain models,
though they are by no means a sufficient set
for all complex cognitive behaviors.
...
\paragraph{4. Neurogrid software (\texttt{nengo\_brainstorm}).}
The Neurogrid backend is a software implementation
of a potential neuromorphic chip being developed
by Boahen et al. \cite{???}.
It does not aim for speed;
...
The SpiNNaker backend targets
the eponymous neuromorphic hardware
developed by Furber et al. \cite{???}.
In contrast to the Neurogrid backend,
...
as do the distilled and Neurogrid backends.
...
The distilled and Neurogrid backends---which
...
the distilled and Neurogrid backends.
...
In this model, the distilled and Neurogrid backends
...
however, we assert that the primary contribution
...
of similar projects, such a PyNN \cite{???},
...
These models are typically build and tested
in the reference backend,
...
{\begin{tabular}{lcc}\toprule
 ...
 Neurogrid (\texttt{nengo\_brainstorm}) & 107 & 50.4\% \\\botrule
\end{tabular}}{}
\end{table}
...
\textbf{\refstepcounter{figure}\label{fig:04} Figure \arabic{figure}.}
       {Basal Ganglia sequence model. (Top) One example instance
         ...
         for four second, allowing for approximately 90 transitions, depending
