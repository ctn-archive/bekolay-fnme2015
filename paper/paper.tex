...
with similar firing rate and inter-spike intervals
...
but around three times as slow as
a C implementation.
...
In particular, this allows SpiNNaker
to take advantage of Nengo's
encoding and decoding capabilities,
greatly reducing the amount of RAM needed
\citep{mundy2015}.
The SpiNNaker backends accomplishes this
by using decoded values to determine
changes to the input currents to each neuron,
rather than using purely
spike-based transmission.
However, a previous implementation
of a Nengo SpiNNaker backend
\citep{galluppi2012}
used primarily spike-based communication,
only making an explicit signal-to-spike
encoding at the beginning
of the network and an explicit
spike-to-signal decoding
...
like the one used in \citealp{galluppi2012},
...
significantly higher compliance in large part
...
is in how each backend implement synaptic filtering.
The reference and OpenCL backends,
for example,
implement arbitrary linear time-invariant filters,
of which the lowpass filter used in this model
is a specific example.
The Brianstorm and distilled backends
only support lowpass filters,
...
in this situation.
However, accuracy could be improved
by increasing the representational range,
though doing so may necessitate an increase
in the number of evaluation points generated
...
Again, the more generic synaptic filtering model
used in the reference and OpenCL backends
is the likely cause of increased variability
on this model.
It is also likely that performance at the extremes
...
In this example, differences in synaptic filtering
...
This result indicates that the synaptic filtering model
of SpiNNaker may also warrant closer examination
for accuracy.
